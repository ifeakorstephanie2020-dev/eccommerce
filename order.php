...
// Get all orders with product details - Updated to use product_id
$sql = "SELECT orders.id, orders.email, orders.product_id, orders.order_date, orders.ip_address, orders.user_agent,
    ...
    <div class="container">
        <center>
            ...
            <p style="color: var(--text-secondary); font-style: italic;">
                Welcome <?php echo htmlspecialchars($_SESSION["name"]); ?> — View all customer orders below
            </p>
        </center>
                    ...
                    <tbody>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><strong style="color: var(--primary-dark);"><?php echo htmlspecialchars($row['name'] ?: '⚠️ Product Deleted'); ?></strong></td>
                                <td><?php echo htmlspecialchars($row['description'] ?: 'N/A'); ?></td>
                                <td><?php echo $row['price'] ? '$' . htmlspecialchars($row['price']) : 'N/A'; ?></td>
                                <td>
                                    <a href="mailto:<?php echo htmlspecialchars($row['email']); ?>" style="color: var(--primary-dark);">
        ...
        <?php if ($result && $result->num_rows > 0): ?>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: var(--spacing-md); margin-top: var(--spacing-xl);">
                ...
                <?php
                // Get total revenue
                $sql_revenue = "SELECT SUM(products.price) as total FROM orders LEFT JOIN products ON orders.product_id = products.id";
