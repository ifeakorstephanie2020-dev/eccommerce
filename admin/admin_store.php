...
$sql = "SELECT p.*,
        (SELECT COUNT(*) FROM orders WHERE id2 = p.id) as order_count
        FROM products p
        ORDER BY p.id DESC";
...
$stats_sql = "SELECT
    ...
    (SELECT IFNULL(SUM(price), 0) FROM orders LEFT JOIN products ON orders.id2 = products.id) as total_revenue";
    ...
    <div class="container">
        <center>
            ...
            <p style="color: var(--text-secondary); font-style: italic;">
                Welcome <?php echo $_SESSION["name"]; ?> — View your store from the admin perspective
            </p>
        </center>
            ...
            <?php if ($results && $results->num_rows > 0): ?>
                <?php while ($row = $results->fetch_assoc()): ?>
                    <article style="position: relative; padding: var(--spacing-lg); background: var(--bg-card); border: 2px solid var(--border-color);">
                        ...
                        <center>
                            <img src="https://picsum.photos/200/200" alt="<?php echo $row["name"]; ?>" style="width: 100%; max-width: 200px; height: auto; border: 2px solid var(--border-color); filter: sepia(0.3);" />
                        </center>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--spacing-sm); margin-top: var(--spacing-md);">
                            ...
                            <a href="delete.php?delete_id=<?php echo $row['id']; ?>"
                               onclick="return confirm('⚠️ Delete <?php echo $row['name']; ?>?')"
